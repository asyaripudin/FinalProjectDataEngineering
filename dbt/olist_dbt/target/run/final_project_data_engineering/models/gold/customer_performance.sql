        ...
        insert into `DataEngineeringDB`.`gold_customer_performance__dbt_backup`
...
WITH customers AS (
    SELECT * FROM `DataEngineeringDB`.`silver_dim_customer`
),

orders AS (
    SELECT * FROM `DataEngineeringDB`.`silver_fact_orders`
),

items AS (
    SELECT * FROM `DataEngineeringDB`.`silver_fact_order_items`
),
...
payments AS (
    ...
    FROM `DataEngineeringDB`.`silver_fact_order_payments`
    GROUP BY order_id
),
...
SELECT
    ...
    THEN dateDiff(
        'day',
        parseDateTimeBestEffortOrNull(first_order_date),
        parseDateTimeBestEffortOrNull(last_order_date)
    )
    ELSE 0
