        ...
        insert into `DataEngineeringDB`.`gold_seller_performance__dbt_backup`
...
WITH source AS (
    SELECT * FROM `DataEngineeringDB`.`silver_dim_sellers`
),
...
order_items AS (
    ...
    FROM `DataEngineeringDB`.`silver_fact_order_items`
    GROUP BY seller_id
),
...
reviews AS (
    ...
    FROM `DataEngineeringDB`.`silver_fact_reviews` r
    JOIN `DataEngineeringDB`.`silver_fact_order_items` oi
