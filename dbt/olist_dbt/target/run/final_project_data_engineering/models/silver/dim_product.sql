        ...
        insert into `DataEngineeringDB`.`silver_dim_products__dbt_backup`
...
WITH source_product AS (
    SELECT * FROM `DataEngineeringDB`.`stg_dim_products`
),

source_category AS (
    SELECT * FROM `DataEngineeringDB`.`stg_dim_product_category_translation`
),
